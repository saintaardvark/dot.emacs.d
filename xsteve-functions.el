...
(defun xsteve-find-in-files ()
  "Find regexp in files using grep."
  (interactive)
  (let* ((regexp (xsteve-get-symbol-at-point  "Find in Files (regex): " t))
         (grep-default-arg (concat "-n -i --directories=skip" (mapconcat '(lambda (arg) (concat " --exclude=" arg))  xsteve-find-in-files-excludes "")))
         (grepcmd
...
(unless (fboundp 'substring-no-properties)
  (defun substring-no-properties (string)
    (substring string 0)))
;18.02.2000
(defun xsteve-get-symbol-at-point (&optional msg-prompt prompt-always)
  (interactive)
  (let* ((region-string (if mark-active
         ...
         (symbol (cond
                  (mark-active
                   (progn
                     (setq region-string (regexp-quote region-string))))
...
(defun xsteve-split-window ()
  ...
  (if (eq last-command 'xsteve-split-window)
    ...
    (let ((buf-list)
      ...
      (switch-to-buffer (cadr buf-list))
...
- With a prefix argument = 0: calculate the best font size to avoid line splitting
  ...
  (let* ((ps-font-size (if (eq nup 0)
                         ...
                         (if (and nup (< nup 0))
                             (- nup)
                           ps-font-size)))
         (nup (or (and nup (> nup 0)) (string-to-number (read-string "N-up printing: " "1"))))
